...
describe 'ConcMysql2' do
  ...
  describe 'Pool' do
    ...
    let(:pool) do
      ...
      IO.should_receive(:open).and_return(*ios)
    ...
    describe 'initialize' do
      ...
      it 'should initialize mysql clients with the given options' do
        Mysql2::Client.should_receive(:new).with(foo: 'foo', bar: 'bar').and_return(*clients)
        IO.should_receive(:open).and_return(*ios)
    ...
    describe 'execute' do
      ...
      it 'should block until a client is ready if all clients are busy' do
        ...
        IO.should_receive(:select).with(ios, nil, nil, anything).and_return([[ios.first]])
